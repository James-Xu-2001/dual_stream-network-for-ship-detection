...
class LetterBoxOBB(LetterBox):
    """Resize image and padding for oriented bounding box (OBB) detection.

    This class extends LetterBox to handle 8-point oriented bounding boxes (OBB) instead of standard 4-point boxes.
    It applies the same resize and pad operations but correctly transforms OBB coordinates.
    ...
    Methods:
        __call__: Resize and pad image, update OBB labels.
        _update_labels: Update OBB coordinates after letterboxing.

    Examples:
        >>> transform = LetterBoxOBB(new_shape=(640, 640))
        >>> result = transform(labels={"img": img, "instances": {"obboxes": obb, "cls": cls}})
        >>> resized_img = result["img"]
        >>> updated_obboxes = result["instances"]["obboxes"]
    """

    def __call__(self, labels: dict[str, Any] | None = None, image: np.ndarray = None) -> dict[str, Any] | np.ndarray:
        """Resize and pad an image with oriented bounding boxes.

        This method applies letterboxing while correctly transforming 8-point OBB coordinates.

        Args:
            labels (dict[str, Any] | None): A dictionary containing image data and OBB labels.
            image (np.ndarray | None): The input image as a numpy array.

        Returns:
            (dict[str, Any] | np.ndarray): Updated labels with resized image and transformed OBB coordinates.
        """
        if labels is None:
            labels = {}
        img = labels.get("img") if image is None else image
        ...
        if len(labels):
            # Pass target image dimensions to _update_labels for proper clipping
            labels = self._update_labels(labels, ratio, left, top, new_shape)
            labels["img"] = img
            labels["resized_shape"] = new_shape
            return labels
        else:
    ...
    def _update_labels(labels: dict[str, Any], ratio: tuple[float, float], padw: float, padh: float, 
                       target_shape: tuple[int, int] = None) -> dict[str, Any]:
        """Update OBB coordinates after applying letterboxing.

        This method transforms 8-point OBB coordinates through the letterboxing operation.
        
        Transformation logic:
        - Input OBB coordinates are in pixel space relative to original image size
        - Step 1: Apply resize scale (multiply by ratio)
        - Step 2: Add padding offset
        - Output: Pixel coordinates in the new padded image
        
        Mathematically:
        - pixel_new = pixel_old * ratio + padding

        Args:
            labels (dict[str, Any]): A dictionary containing image labels with 'instances' key.
                instances should contain 'obboxes' (N, 8) array and 'cls' (N,) array.
                OBB coordinates should be in pixel space (not normalized).
            ratio (tuple[float, float]): Scaling ratios (width, height) applied to the image.
                ratio = (new_w_unpad / old_w, new_h_unpad / old_h)
            padw (float): Padding width added to the image (left side).
            padh (float): Padding height added to the image (top side).
            target_shape (tuple[int, int], optional): Target image shape (height, width) after resize+pad.
                If provided, coordinates will be clipped to this shape.

        Returns:
            (dict[str, Any]): Updated labels dictionary with transformed OBB coordinates in pixel space.

        Examples:
            >>> letterbox = LetterBoxOBB(new_shape=(640, 640))
            >>> labels = {"instances": {"obboxes": np.array([[10, 10, 20, 10, 20, 20, 10, 20]]), "cls": [0]}}
            >>> ratio = (6.4, 6.4)
            >>> padw, padh = 0, 0
            >>> updated_labels = letterbox._update_labels(labels, ratio, padw, padh, (640, 640))
        """
        instances = labels.get("instances")
        if instances is None:
            return labels

        obboxes = instances.get("obboxes")
        if obboxes is None or len(obboxes) == 0:
            return labels

        # Separate x and y coordinates (8 coords: x1,y1,x2,y2,x3,y3,x4,y4)
        # Even indices (0,2,4,6) are x, odd indices (1,3,5,7) are y
        obboxes_transformed = obboxes.copy()

        # Transform pixel coordinates:
        # pixel_new = pixel_old * ratio + padding
        # x coordinates (indices 0, 2, 4, 6)
        obboxes_transformed[:, 0::2] = obboxes[:, 0::2] * ratio[0] + padw
        # y coordinates (indices 1, 3, 5, 7)
        obboxes_transformed[:, 1::2] = obboxes[:, 1::2] * ratio[1] + padh

        # Clip coordinates to target image boundaries if target_shape is provided
        if target_shape is not None:
            target_h, target_w = target_shape
            obboxes_transformed[:, 0::2] = np.clip(obboxes_transformed[:, 0::2], 0, target_w)
            obboxes_transformed[:, 1::2] = np.clip(obboxes_transformed[:, 1::2], 0, target_h)

        # Update instances
        labels["instances"]["obboxes"] = obboxes_transformed
        return labels
